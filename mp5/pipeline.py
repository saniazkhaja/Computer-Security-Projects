...
def vectorize(password):
    ...
    # 1. password length
    vector.append(len(password))

    # 2. number of lowercase letters
    vector.append(sum(1 for char in password if char.islower()))

    # 3. number of uppercase letters
    vector.append(sum(1 for char in password if char.isupper()))

    # 4. number of digits
    vector.append(sum(1 for char in password if char.isdigit()))

    # 5. number of special characters (not a-z, A-Z, 0-9)
    vector.append(sum(1 for char in password if not char.isalnum()))

    # 6. letter-only password? (1 for yes, 0 for no)
    vector.append(1 if password.isalpha() else 0)

    # 7. digit-only password? (1 for yes, 0 for no)
    vector.append(1 if password.isdigit() else 0)
    ...
    vector.append(sum(1 for i in range(1, len(password)) if password[i].lower() == password[i - 1].lower()))

    # 9. max number of consec. letters
    max_consec_letters = max((len(list(g)) for k, g in itertools.groupby(password, key=str.isalpha) if k), default=0)
    vector.append(max_consec_letters)

    # 10. max number of consec. digits
    max_consec_digits = max((len(list(g)) for k, g in itertools.groupby(password, key=str.isdigit) if k), default=0)
    vector.append(max_consec_digits)
    ...
    consec_digits_head = len(list(itertools.takewhile(str.isdigit, password)))
    vector.append(consec_digits_head)

    # 14. number of consec. digits (tail)
    consec_digits_tail = len(list(itertools.takewhile(str.isdigit, reversed(password))))
    vector.append(consec_digits_tail)

    # 15. number of consec. letters (head)
    consec_letters_head = len(list(itertools.takewhile(str.isalpha, password)))
    vector.append(consec_letters_head)

    # 16. number of consec. letters (tail)
    consec_letters_tail = len(list(itertools.takewhile(str.isalpha, reversed(password))))
    vector.append(consec_letters_tail)

    # 17. number of consec. special characters (head)
    consec_special_head = len(list(itertools.takewhile(lambda x: not x.isalnum(), password)))
    vector.append(consec_special_head)

    # 18. number of consec. special characters (tail)
    consec_special_tail = len(list(itertools.takewhile(lambda x: not x.isalnum(), reversed(password))))
    vector.append(consec_special_tail)

    return np.array(vector).reshape(1, 18)
...
def sequential_guessor(password):
    ...
    # guess = None
    # yield guess
    features = vectorize(password)
    rule_probs = model.predict_proba(features)[0]
    rule_order = np.argsort(rule_probs)[::-1]
    
    for rule_index in rule_order:
        rule_name = guess_classes[rule_index]
        transformations = transformation_mat[rule_name]
        apply_transformation = guess_functions[rule_name]
        
        for transformation in transformations:
            # Pass graph for seqkey transformations
            if apply_transformation == apply_seqkey_transformation:
                guesses = apply_transformation(password, transformation, seq_graph)
            else:
                guesses = apply_transformation(password, transformation)
